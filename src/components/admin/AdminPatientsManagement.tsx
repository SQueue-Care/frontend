...
export default function AdminPatientsManagement() {
  ...
  return (
    <div className="animate-in fade-in space-y-6 duration-500">
      <div>
        <h1 className="mb-2 font-['Manrope'] text-3xl font-extrabold text-zinc-950">Data Pasien</h1>
        <p className="text-slate-600">Cari pasien terdaftar dan lihat profil serta ringkasan kunjungan.</p>
      </div>
      ...
      <div className="flex flex-col gap-4 sm:flex-row">
        <input
          ...
          className="flex-1 rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-teal-500 focus:outline-none"
        />
      </div>
        ...
        <div className="lg:col-span-2">
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            {isLoading && patients.length === 0 ? (
              ...
              <p className="py-16 text-center text-sm text-slate-500">Tidak ada pasien ditemukan.</p>
            ) : (
                ...
                {patients.map((p) => (
                  <li key={p.id}>
                    ...
                    >
                      ...
                      {p.bpjsNumber && (
                        <span className="mt-1 inline-block rounded bg-blue-50 px-2 py-0.5 text-[10px] font-bold text-blue-700">
                          BPJS
                        </span>
                      )}
                    </button>
          ...
          {!selectedId ? (
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 className="font-['Manrope'] text-xl font-bold text-zinc-900">{selectedPatient.user.name}</h2>
                <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    <dt className="text-slate-500">BPJS</dt>
                    <dd className="font-medium">{selectedPatient.bpjsNumber || '—'}</dd>
                  </div>
                  <div>
                    <dt className="text-slate-500">Golongan Darah</dt>
                    <dd className="font-medium">{selectedPatient.bloodType || '—'}</dd>
                  </div>
                  <div>
                    <dt className="text-slate-500">Alergi</dt>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <h3 className="mb-4 font-bold text-zinc-900">Riwayat Kunjungan Selesai</h3>
                {!medicalProfile?.medicalHistory?.length ? (
                  <p className="text-sm text-slate-500 italic">Belum ada kunjungan selesai.</p>
                ) : (
                  <ul className="space-y-3">
                    {medicalProfile.medicalHistory.map((visit) => (
                      <li key={visit.id} className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                        <div className="mb-2 flex justify-between gap-2">
                          <div>
                            <p className="text-sm font-bold text-teal-700">{visit.department?.name}</p>
                            <p className="text-xs text-slate-500">
                              {new Date(visit.queueDate).toLocaleDateString('id-ID')} · #{visit.queueNumber}
                            </p>
                          </div>
                          {visit.doctor?.user?.name && (
                            <span className="text-xs font-semibold text-slate-600">{visit.doctor.user.name}</span>
                          )}
                        </div>
