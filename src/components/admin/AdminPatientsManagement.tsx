    ...
    <div className="animate-in fade-in space-y-6 duration-500">
      <div>
        ...
        <p className="text-sm font-medium text-slate-500 dark:text-zinc-400">
          Cari pasien terdaftar dan lihat profil serta ringkasan riwayat medis.
        </p>
      </div>
          ...
          <div className="no-scrollbar relative flex-1 overflow-y-auto">
            {isLoading && !patients.length ? (
              <div className="flex h-full items-center justify-center py-12">
                <div className="h-7 w-7 animate-spin rounded-full border-4 border-slate-100 border-b-teal-600 dark:border-zinc-800 dark:border-b-teal-500" />
              </div>
            ) : patients.length === 0 ? (
                  ...
                  return (
                    <li key={p.id}>
                      <button
                        ...
                        className={`group relative flex w-full flex-col items-start px-5 py-4 text-left transition-all hover:bg-teal-50/50 dark:hover:bg-[#252628] ${
                          isSelected ? 'bg-teal-50/80 dark:bg-teal-500/10' : ''
                        }`}
                      >
                        {isSelected && (
                          <div className="absolute bottom-0 left-0 top-0 w-1 bg-teal-600 dark:bg-teal-500" />
                        )}
                        <p className={`font-medium transition-colors ${isSelected ? 'text-teal-700 dark:text-teal-400' : 'text-zinc-900 dark:text-zinc-100 group-hover:text-teal-600 dark:group-hover:text-teal-400'}`}>
                          {p.user.name}
                        </p>
        ...
        <div className="lg:col-span-2">
          {!selectedId ? (
            <div className="flex min-h-[300px] flex-col items-center justify-center rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center dark:border-zinc-800 dark:bg-[#131314]">
              <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-teal-50 text-teal-600 dark:bg-teal-900/20 dark:text-teal-400">
                <svg className="h-8 w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 21h7a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v11m0 5l4.879-4.879m0 0a3 3 0 104.243-4.242 3 3 0 00-4.243 4.242z" />
                </svg>
              </div>
              <h3 className="font-['Manrope'] text-lg font-bold text-zinc-900 dark:text-zinc-100">Jelajahi Data Pasien</h3>
              <p className="mt-1 text-sm font-medium text-slate-500 dark:text-zinc-400">
                Pilih pasien dari daftar di sebelah kiri untuk melihat profil lengkap.
              </p>
            </div>
          ) : isLoading && !patient ? (
            ...
            <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-[#1e1f20]">
              ...
              <div className="border-b border-slate-100 bg-slate-50/50 p-6 dark:border-zinc-800 dark:bg-[#131314]/50">
                <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                  <div className="flex items-center gap-4">
                    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-teal-100 text-xl font-bold text-teal-700 dark:bg-teal-500/20 dark:text-teal-400">
              ...
              {/* INFORMASI KREDENSIAL EKSTRA UNTUK ADMIN */}
              <div className="border-b border-slate-100 px-6 py-5 dark:border-zinc-800">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                  <div className="rounded-xl bg-slate-50 p-4 dark:bg-[#131314]">
                    <p className="mb-1 text-[10px] font-bold tracking-widest text-slate-500 uppercase dark:text-zinc-500">Alamat Email</p>
                    <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{patient.user.email}</p>
                  </div>
                  <div className="rounded-xl bg-slate-50 p-4 dark:bg-[#131314]">
                    <p className="mb-1 text-[10px] font-bold tracking-widest text-slate-500 uppercase dark:text-zinc-500">Nomor Telepon</p>
                    <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{patient.phone || '-'}</p>
                  </div>
                  <div className="rounded-xl bg-slate-50 p-4 dark:bg-[#131314]">
                    <p className="mb-1 text-[10px] font-bold tracking-widest text-slate-500 uppercase dark:text-zinc-500">Golongan Darah</p>
                    <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{patient.bloodType || '-'}</p>
                  </div>
                </div>
              </div>
