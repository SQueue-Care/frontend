  ...
  return (
    <div className="flex h-full flex-col rounded-2xl border border-slate-200 transition-colors dark:border-zinc-800 bg-white transition-colors dark:bg-[#1e1f20] shadow-sm">
      <div className="border-b border-slate-100 transition-colors dark:border-zinc-800 px-5 py-4">
        <h3 className="font-['Manrope'] text-lg font-extrabold text-zinc-900 transition-colors dark:text-zinc-100 ">Riwayat Medis</h3>
            ...
            {pastVisits.map((visit) => (
              ...
              >
                <div className="mb-2 flex items-start justify-between gap-2">
                  <div>
                    <p className="text-xs font-bold text-indigo-700 transition-colors dark:text-indigo-400 ">
                  ...
                  {visit.doctor?.user?.name && (
                    <span className="shrink-0 rounded-md bg-white transition-colors dark:bg-[#1e1f20] px-2 py-0.5 text-[10px] font-semibold text-slate-600 transition-colors dark:text-zinc-300 ">
                      {visit.doctor.user.name}
                    </span>
                  )}
                </div>
                {visit.doctorNotes?.diagnosis ? (
                  <p className="text-sm font-semibold text-zinc-900 transition-colors dark:text-zinc-100 ">{visit.doctorNotes.diagnosis}</p>
                ) : (
