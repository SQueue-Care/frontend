    ...
    <div className="animate-in fade-in duration-500">
      ...
      <div className="rounded-2xl border border-slate-200 transition-colors dark:border-zinc-800 bg-white transition-colors dark:bg-[#1e1f20] p-6 shadow-sm">
        {isLoading ? (
          ...
          <div className="py-12 text-center text-sm font-bold text-rose-600 transition-colors dark:text-rose-400 italic">{error}</div>
        ) : results.length === 0 ? (
              ...
              >
                <div className="mb-2 flex items-start justify-between">
                  <div className="flex-1">
                    <h3 className="font-bold text-zinc-900 transition-colors dark:text-zinc-100 ">
                  ...
                  <span className="rounded-md border border-violet-200 bg-violet-50 transition-colors dark:bg-violet-900/20 px-2.5 py-1 text-xs font-bold text-violet-600 transition-colors dark:text-violet-400 ">
                    {result.candidates?.length || 0} diagnoses
                  </span>
                </div>

                <div className="mb-2">
                  <p className="mb-2 text-xs font-semibold text-slate-600 transition-colors dark:text-zinc-300 ">Gejala:</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white transition-colors dark:bg-[#1e1f20] shadow-lg">
            ...
            <div className="space-y-4 p-6">
              <div>
                <p className="mb-1 text-sm font-semibold text-slate-700 transition-colors dark:text-zinc-300 ">Pasien</p>
                <p className="text-base font-bold text-zinc-900 transition-colors dark:text-zinc-100 ">
                  {selectedResult.patient?.user?.name || 'Unknown'}
                </p>
              </div>

              <div>
                <p className="mb-1 text-sm font-semibold text-slate-700 transition-colors dark:text-zinc-300 ">Tanggal Analisis</p>
                <p className="text-sm text-slate-600 transition-colors dark:text-zinc-300 ">{formatDate(selectedResult.createdAt)}</p>
              </div>

              <div>
                <p className="mb-2 text-sm font-semibold text-slate-700 transition-colors dark:text-zinc-300 ">Catatan</p>
              ...
              <div>
                <p className="mb-2 text-sm font-semibold text-slate-700 transition-colors dark:text-zinc-300 ">Gejala Teridentifikasi</p>
                <div className="flex flex-wrap gap-2">
                  {selectedResult.symptoms.map((symptom, idx) => (
                    <span
                      key={idx}
                      className="rounded-full border border-emerald-200 transition-colors dark:border-emerald-500/20 bg-emerald-50 transition-colors dark:bg-emerald-500/10 px-3 py-1.5 text-sm font-semibold text-emerald-700"
                    >
                      {symptom}
                    </span>
                  ))}
              ...
              <div>
                <p className="mb-3 text-sm font-semibold text-slate-700 transition-colors dark:text-zinc-300 ">Kandidat Diagnosis</p>
                  ...
                  {selectedResult.candidates.map((candidate, idx) => (
                    <div key={idx} className="rounded-lg border border-slate-200 transition-colors dark:border-zinc-800 bg-slate-50 transition-colors dark:bg-[#131314] p-3">
                      <div className="mb-2 flex items-start justify-between">
                        ...
                        {candidate.urgency && (
                          <span className="rounded-md border border-violet-200 bg-violet-50 transition-colors dark:bg-violet-900/20 px-2 py-1 text-xs font-bold text-violet-600 transition-colors dark:text-violet-400 ">
