...
function DetailItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-slate-100 transition-colors dark:border-zinc-800 bg-slate-50/80 transition-colors dark:bg-[#131314]/80 p-3">
      <p className="mb-0.5 text-[10px] font-bold tracking-wider text-slate-400 transition-colors dark:text-zinc-500 uppercase">
        {label}
      </p>
      <p className="text-sm font-semibold text-zinc-900 transition-colors dark:text-zinc-100 ">{value}</p>
    </div>
  )
}
    ...
    <div className="overflow-hidden rounded-2xl border border-indigo-100 transition-colors dark:border-indigo-800/50 bg-white transition-colors dark:bg-[#1e1f20] shadow-sm">
      <div className="relative border-b border-indigo-100 transition-colors dark:border-indigo-800/50 bg-gradient-to-br from-indigo-600 via-indigo-700 to-violet-800 px-6 py-5 text-white">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl border-2 border-white/30 bg-white/15 text-2xl font-black backdrop-blur-sm">
              {getPatientInitials(name)}
            </div>
            <div>
              <span className="mb-1 inline-flex items-center rounded-full border border-white/25 bg-white/10 px-2.5 py-0.5 text-[10px] font-bold tracking-wide uppercase">
                {statusLabel}
              </span>
          ...
          <div className="rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-center backdrop-blur-sm">
            <p className="text-[10px] font-bold tracking-wider text-indigo-200 uppercase">
              No. Antrean
            </p>
      ...
      {allergies && (
        <div className="flex items-start gap-3 border-b border-rose-100 transition-colors dark:border-rose-500/20 bg-rose-50 transition-colors dark:bg-rose-500/10 px-6 py-3">
          ...
          <div>
            <p className="text-xs font-bold tracking-wide text-rose-700 transition-colors dark:text-rose-400 uppercase">Alergi</p>
      ...
      <div className="p-6">
        ...
        {queue.notes?.trim() && (
          <div className="mb-6 rounded-xl border border-amber-100 transition-colors dark:border-amber-500/20 bg-amber-50/70 p-4">
            <p className="mb-1 text-[10px] font-bold tracking-wider text-amber-700 transition-colors dark:text-amber-400 uppercase">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            onClick={onOpenCdss}
            className="rounded-lg border border-violet-200 bg-violet-50 transition-colors dark:bg-violet-900/20 px-4 py-2 text-xs font-bold text-violet-700 transition-colors hover:bg-violet-100"
          >
            Analisis CDSS
          </button>
          <button
            type="button"
            onClick={onOpenNotes}
            className="rounded-lg border border-amber-200 transition-colors dark:border-amber-500/20 bg-amber-50 transition-colors dark:bg-amber-500/10 px-4 py-2 text-xs font-bold text-amber-800 transition-colors hover:bg-amber-100"
          >
            Catatan Dokter
          </button>
