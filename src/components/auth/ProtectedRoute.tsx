...
export default function ProtectedRoute({ allowedRoles }: ProtectedRouteProps) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 font-['Inter']">
        <div className="text-center">
          ...
          <p className="text-sm font-medium text-slate-500">Memverifikasi sesi aman...</p>
        </div>
      </div>
    );
  }
  ...
  // Cegah akses jika pengguna sudah login tetapi rolenya tidak diizinkan
  if (allowedRoles && allowedRoles.length > 0 && !allowedRoles.includes(user.role)) {
    // Alihkan berdasarkan role mereka untuk menghindari infinite loop
