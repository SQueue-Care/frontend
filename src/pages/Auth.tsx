...
// Baris 1: Tema Pemeriksaan / Kesehatan (Detak Jantung, Hati)
const row1Icons = [
  // Heartbeat (Activity)
...
// Baris 2: Tema Administrasi / Antrean (Berkas, Stetoskop, Folder)
const row2Icons = [
  // Document text
...
// Baris 3: Tema Fasilitas / Bangunan (Bangunan, Home/Klinik, Pill/Obat)
const row3Icons = [
  // Building office
...
// Baris 4: Tema Waktu / Petugas (Jam, Orang/Pasien, Badge ID)
const row4Icons = [
  // Clock
...
export default function Auth() {
  ...
  // State animasi untuk pergantian mode (Login/Register)
  const [isTransitioning, setIsTransitioning] = useState(false)
  const [isFormFocused, setIsFormFocused] = useState(false)
  
  // State untuk mount awal menyerupai PatientPortal
  ...
  useEffect(() => {
    // Sinkronisasi render awal dengan animasi masuk
  ...
  const toggleMode = (newMode: AuthMode) => {
    ...
    }, 250) // Waktu transisi diselaraskan ke 250ms untuk kehalusan maksimal
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({
        api: getErrorMessage(error, 'Terjadi kegagalan komunikasi dengan peladen.'),
      })
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    // INJEKSI MUTLAK: antialiased ditambahkan untuk rendering tipografi yang sangat tajam
    <div className="min-h-screen bg-slate-50 dark:bg-[#131314] flex items-center justify-center relative overflow-hidden transition-colors duration-500 font-['Inter'] antialiased px-4 py-8">
      ...
      {/* LATAR BELAKANG MOTIF BATIK (POLA PAPAN CATUR/STAGGERED) */}
      <div className={`fixed inset-0 z-0 pointer-events-none transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] flex items-center justify-center ${isFormFocused ? 'blur-md scale-105 opacity-40' : 'blur-none scale-100 opacity-100'}`}>
        <div className="w-full h-full flex flex-col justify-between overflow-hidden opacity-70 p-4">
          {Array.from({ length: 12 }).map((_, rowIndex) => {
            // Tentukan kelompok ikon berdasarkan baris (0,1,2,3 -> mengulang)
            ...
            return (
              <div key={`row-${rowIndex}`} className="flex w-full justify-between items-center">
                {Array.from({ length: 14 }).map((_, colIndex) => {
                  // Logika checkerboard:
                  // Jika hasil jumlah baris & kolom adalah ganjil, maka kosongkan (ruang spasi)
                  // Jika genap, maka tampilkan ikon.
                  if ((rowIndex + colIndex) % 2 !== 0) {
                    return <div key={`cell-${rowIndex}-${colIndex}`} className="w-8 h-8" />;
                  }

                  const path = iconSet[colIndex % iconSet.length];
                  // Rotasi divariasikan agar tidak terlalu kaku
      ...
      >
        ...
        <div className={`transition-all duration-250 ease-in-out ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
          <div className="mb-8 text-center">
            {/* INJEKSI: Hierarki font heading diperbaiki (tracking-tight standar Google) */}
          ...
          >
            ...
            <div className="mt-5">
              <button 
                type="submit" 
                disabled={isLoading}
                // INJEKSI: Mengurangi uppercase berlebih, mengutamakan font-medium standar profesional
          ...
          <button 
            type="button"
            // INJEKSI: Desain SSO disesuaikan dengan Google Material Guidelines (warna teks netral, font medium)
