...
export default function Auth() {
  ...
  const toggleMode = (newMode: AuthMode) => {
    ...
    setTimeout(() => {
      setMode(newMode)
      setErrors({})
      setShowPassword(false) // Reset state password toggle saat pindah mode
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // PENINGKATAN: Deteksi error status HTTP spesifik dari API
        ...
        <div className={`transition-all duration-250 ease-in-out ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
          ...
          >
            ...
            <div className="flex flex-col">
              {/* PENINGKATAN: Pembungkus relatif untuk visibilitas kata sandi */}
