...
const getHistoryStatusStyle = (status: string) => {
  switch (status) {
    ...
    case 'DONE':
    case 'SKIPPED':
    case 'CANCELLED': return 'bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-500/10 dark:text-slate-400 dark:border-slate-500/20';
    default: return 'bg-slate-50 text-slate-500 border-slate-200 dark:bg-slate-500/10 dark:text-slate-400 dark:border-slate-500/20';
  }
};
...
const getAppointmentStatusStyle = (status: string) => {
  switch (status) {
    case 'BOOKED': return 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20';
    case 'CONFIRMED': return 'bg-teal-50 text-teal-700 border-teal-200 dark:bg-teal-500/10 dark:text-teal-400 dark:border-teal-500/20';
    case 'COMPLETED':
    case 'CANCELLED': return 'bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-500/10 dark:text-slate-400 dark:border-slate-500/20';
    default: return 'bg-slate-50 text-slate-500 border-slate-200 dark:bg-slate-500/10 dark:text-slate-400 dark:border-slate-500/20';
  }
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-[#131314] font-['Inter'] relative transition-colors duration-500 overflow-x-hidden">
          ...
          <main className="relative z-10 w-full max-w-[1440px] mx-auto px-6 sm:px-8 lg:px-15 py-10 md:py-12 flex-1 mt-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  ...
                  {filteredPolyclinics.length === 0 && (
                    <div className="col-span-1 md:col-span-2 lg:col-span-3 text-center py-16">
                      <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-100 dark:bg-[#1e1f20] text-slate-400 dark:text-zinc-500 mb-4 transition-colors">
                        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                      </div>
                      <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-1 transition-colors">Layanan tidak ditemukan</h3>
                      <p className="text-sm text-slate-500 dark:text-zinc-400 transition-colors">Kami tidak dapat menemukan poliklinik dengan kata kunci "{searchQuery}".</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {activeView === 'polyclinics' && (
              <div className="flex flex-col items-center justify-center py-32 text-center animate-in fade-in zoom-in-95 duration-500">
                <svg className="w-20 h-20 text-slate-300 dark:text-slate-700 animate-[spin_3s_linear_infinite] mb-6 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <h2 className="text-2xl font-black text-zinc-900 dark:text-white tracking-tight mb-2 transition-colors">Fitur Sedang Dibangun</h2>
                <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto transition-colors">Halaman manajemen poliklinik ini sedang dalam tahap pengembangan teknis. Silakan kembali lagi nanti.</p>
              </div>
            )}
            ...
            {activeView === 'reservations' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out space-y-6">
                <div className="mb-4">
                  <h1 className="text-3xl font-extrabold text-zinc-950 dark:text-white font-['Manrope'] tracking-tighter mb-2 transition-colors">Jadwal Reservasi</h1>
                    ...
                    <table className="w-full text-left border-collapse min-w-[900px]">
                      ...
                      <tbody className="text-sm divide-y divide-slate-100 dark:divide-zinc-800">
                        {isLoadingAppointments ? (
                          <tr><td colSpan={5} className="p-16 text-center animate-pulse text-teal-700 dark:text-teal-500 font-bold uppercase tracking-widest text-xs bg-slate-50/50 dark:bg-slate-800/50 transition-colors">Menyinkronkan reservasi...</td></tr>
                        ) : patientAppointments.length === 0 ? (
                          <tr><td colSpan={5} className="p-16 text-center text-slate-400 dark:text-slate-500 italic font-medium bg-slate-50/50 dark:bg-slate-800/50 transition-colors">Tidak ada jadwal reservasi aktif saat ini.</td></tr>
                        ) : (
                          patientAppointments.map((apt: any) => (
                            <tr 
                              key={apt.id}
                              className="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-all duration-200 group cursor-pointer active:bg-slate-100/70 dark:active:bg-slate-700/50"
                              onClick={() => {
                            ...
                            >
                              <td className="p-6 pl-8 align-top">
                                <div className="font-extrabold text-zinc-900 dark:text-white text-base mb-1 transition-colors">{apt?.department?.name || 'Poliklinik'}</div>
                                <div className="text-[11px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wide transition-colors">{apt?.doctor?.user?.name || 'Dokter belum ditentukan'}</div>
                              </td>
                              <td className="p-6 align-top">
                                <div className="text-zinc-900 dark:text-white font-extrabold mb-1 transition-colors">
                                  {new Date(apt.scheduledAt).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
                                </div>
                                <div className="inline-flex px-2 py-1 bg-slate-100 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400 rounded-md text-[10px] font-black uppercase tracking-widest transition-colors">
                                  Sesi: {new Date(apt.scheduledAt).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })} WIB
                                </div>
                              </td>
                              <td className="p-6 align-top">
                                <div className="max-w-[220px] text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed line-clamp-3 transition-colors" title={apt.notes}>
                                  {apt.notes || <span className="italic text-slate-400 dark:text-slate-500">Tidak ada catatan keluhan.</span>}
                                </div>
                              </td>
                              <td className="p-6 align-top">
                                <span className="text-xs text-slate-500 dark:text-slate-400 font-bold transition-colors">{new Date(apt.createdAt).toLocaleDateString('id-ID')}</span>
                              </td>
            ...
            {activeView === 'queues' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out space-y-6">
                <div className="mb-4">
                  <h1 className="text-3xl font-extrabold text-zinc-950 dark:text-white font-['Manrope'] tracking-tighter mb-2 transition-colors">Riwayat Kunjungan</h1>
                    ...
                    <table className="w-full text-left border-collapse min-w-[900px]">
                      ...
                      <tbody className="text-sm divide-y divide-slate-100 dark:divide-zinc-800">
                        {isLoadingTable ? (
                          ...
                          <tr><td colSpan={5} className="p-16 text-center text-slate-400 dark:text-slate-500 italic font-medium bg-slate-50/50 dark:bg-slate-800/50 transition-colors">Tidak ada riwayat kunjungan yang terekam.</td></tr>
                        ) : (
                          patientHistory.map((item) => (
                            ...
                            >
                              <td className="p-6 pl-8 align-top">
                                <div className="font-extrabold text-zinc-900 dark:text-white text-base mb-1 transition-colors">{item?.department?.name || 'Poliklinik'}</div>
                                <div className="text-[11px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wide transition-colors">{item?.doctor?.user?.name || 'Dokter belum ditentukan'}</div>
                              </td>
                              <td className="p-6 align-top">
                                <div className="text-zinc-900 dark:text-white font-extrabold transition-colors">
                                  {new Date(item.queueDate).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}
                                </div>
                              </td>
                              <td className="p-6 align-top">
                                <div className="inline-flex px-2.5 py-1 bg-slate-100 dark:bg-slate-900/50 text-slate-600 dark:text-slate-400 rounded-md text-[10px] font-black uppercase tracking-widest transition-colors">
                                  {new Date(item.queueDate).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })} WIB
                                </div>
                              </td>
                              <td className="p-6 align-top">
                                <div className="max-w-[200px] text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed line-clamp-3 transition-colors" title={item.notes}>
                                  {item.notes || <span className="italic text-slate-400 dark:text-slate-500">Tidak ada catatan keluhan.</span>}
                                </div>
                              </td>
                              <td className="p-6 text-right pr-8 align-top">
        ...
        {isNikWarningOpen && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[80] flex items-center justify-center p-4 animate-in fade-in duration-300">
            <div className="bg-white dark:bg-[#1e1f20] rounded-3xl shadow-2xl max-w-sm w-full border border-slate-200 dark:border-zinc-800 overflow-hidden text-center animate-in zoom-in-95 duration-300 transition-colors">
              <div className="p-8">
                <div className="w-16 h-16 bg-amber-100 dark:bg-amber-900/40 text-amber-500 dark:text-amber-400 rounded-full flex items-center justify-center mx-auto mb-5 shadow-inner transition-colors">
                  <svg className="w-8 h-8" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
                </div>
                
                <h3 className="text-xl font-black text-zinc-900 dark:text-white tracking-tight mb-2 transition-colors">Akses Layanan Ditahan</h3>
                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium mb-8 leading-relaxed transition-colors">
                  Anda diwajibkan untuk melengkapi <strong className="text-zinc-800 dark:text-slate-200">Nomor Induk Kependudukan (NIK)</strong> pada profil medis Anda sebelum dapat membuat antrean atau reservasi.
                </p>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <button
                    ...
                    className="w-full px-4 py-3.5 text-slate-500 dark:text-zinc-400 font-extrabold hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-xl transition-all active:scale-95"
                  >
                    Nanti Saja
                  </button>
                </div>
