...
function DetailRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-4 text-sm">
      ...
      <span className="text-right font-extrabold text-zinc-950 dark:text-zinc-100">{value}</span>
    </div>
  )
}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h4 className="mb-3 border-b border-slate-200 pb-2 text-[10px] font-black tracking-widest text-slate-400 uppercase dark:border-zinc-800 dark:text-zinc-500">
        {title}
      </h4>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6 duration-700 ease-out">
      ...
      <div className="flex items-start gap-4">
        <button
          ...
          className="mt-1 flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-black tracking-wide text-slate-600 shadow-sm transition-all hover:bg-slate-50 active:scale-95 dark:border-zinc-800 dark:bg-[#1e1f20] dark:text-zinc-300 dark:hover:bg-zinc-800"
        >
      ...
      {isLoading && (
        <div className="flex items-center justify-center rounded-3xl border border-slate-200 bg-white p-16 shadow-sm dark:border-zinc-800 dark:bg-[#1e1f20]">
          <div className="text-center">
            ...
            <p className="text-xs font-bold tracking-widest text-teal-700 uppercase dark:text-teal-500">
              Memuat detail...
            </p>
          </div>
        </div>
      ...
      {!isLoading && error && (
        <div className="rounded-3xl border border-red-200 bg-red-50 p-10 text-center dark:border-red-900/50 dark:bg-red-900/10">
          ...
          <button
            onClick={() => navigate('/portal/queues')}
            className="mt-4 rounded-xl bg-slate-100 px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-200 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700"
          >
            Kembali ke Riwayat
          </button>
        </div>
      ...
      {!isLoading && queue && (
        <div className="grid gap-5 lg:grid-cols-3">
          ...
          <div className="flex items-center justify-between rounded-3xl border border-slate-100 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-[#1e1f20] lg:col-span-3">
            <div>
              <p className="mb-2 text-[10px] font-black tracking-widest text-slate-400 uppercase dark:text-zinc-500">
                Nomor Urut Antrean
              </p>
              <span className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-slate-50 px-6 py-3 font-mono text-4xl font-black text-zinc-900 shadow-sm dark:border-zinc-700 dark:bg-[#131314] dark:text-zinc-100">
                {queue.queueNumber}
              </span>
            </div>
            <div className="text-right">
              ...
              <span
                className={`inline-flex rounded-xl border px-5 py-2.5 text-[11px] font-black tracking-widest uppercase ${STATUS_STYLE[queue.status] ?? 'border-slate-200 bg-slate-50 text-slate-500'}`}
              >
